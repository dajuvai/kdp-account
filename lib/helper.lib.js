...
*/
const generateKey = (a, d) =>
  encryption.encrypt(`${generateString(4)}::${a}::${d}::${generateString(4)}`);
...
module.exports.createClientObject = (d, n) => {
  ...
  return {
    client_id: x,
    client_secret: `${generateString(3)}-${generateString(4)}-${generateString(
      3,
    )}`,
    client_name: n,
    client_domain: d,
...
module.exports.createAdminObject = b => {
  console.log('Admin object==', b);
  return { ...b, password: encryption.encrypt(b.password) };
};
module.exports.handleError = (res, body) => {
